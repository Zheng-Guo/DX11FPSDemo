...
Sphere::Sphere(ID3D11Device* device) :Shape(device)
{
	type = ShapeType::Sphere;
	GeometryManager* geoManager = GeometryManager::getInstance();
	numberOfIndices = geoManager->getSphereIndexCount();
	...
	vertexBufferDesc.ByteWidth = vertexSize*geoManager->getSphereVertexCount();
	vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vertexBufferDesc.CPUAccessFlags = 0;
	vertexBufferDesc.MiscFlags = 0;

	ZeroMemory(&vertexBufferData, sizeof(vertexBufferData));
	vertexBufferData.pSysMem = geoManager->getSphereVertices();
	...
	iinitData.pSysMem = geoManager->getSphereIndices();
...
void Sphere::draw(ID3D11DeviceContext * deviceContext, CXMMATRIX camView, CXMMATRIX camProjection)
{
...
void Sphere::setRotation(float x, float y, float z)
{
